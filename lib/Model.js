...
*/
class Model extends lib.node.events.EventEmitter {
	/**
	...
	*/
	static create(p_config) {
		...
		let schema = new lib.odin.Schema(config.abstract, config.fields, config.parent.schema);
		let model = class extends config.parent {
			constructor(p_data) {
				super(schema, p_data);
